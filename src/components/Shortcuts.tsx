        ...
        <div className="max-h-[62vh] overflow-auto p-4">
          {groups.map((g) => (
            <section key={g.heading} className="mb-5 last:mb-0">
              <h3 className="mb-2 text-[13px] tracking-[0.15em] text-[var(--color-dim)]">{g.heading}</h3>
              <ul className="flex flex-col">
                {g.items.map((item, i) => (
                  ...
                  >
                    ...
                    <kbd className="shrink-0 rounded-[4px] border border-[var(--color-line)] bg-[var(--color-bg)] px-2 py-0.5 text-[13px] tracking-[0.08em] text-[var(--color-fg)]">
                      {item.keys}
                    </kbd>
                  </li>
                ))}
